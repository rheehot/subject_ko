...
이 프로젝트는 여러분의 collection에 매우 편리한 함수를 추가하는 것을 허용하지 않을 것입니다. 그렇지만 C 프로그래밍에서 매우 흥미로운 새 개념을 배울 수 있을 것입니다.
...
```
노랭이
get_next_line이 호출될 때마다 가능한 한 적게 읽도록 해야 합니다. 만약 newline을 만나면, 현재라인을 반환해야 합니다. 전체 파일을 읽지 말고 각 라인을 처리하세요.
```

```
노랭이
테스트하지 않고 프로젝트를 제출하지 마세요. Cover your bases. 실행할 많은 테스트 프로그램들이 있습니다. 파일로부터, redirection으로부터, stdin으로부터의 읽기를 시도하세요. 표준 출력에 newline을 보낼 때 프로그램은 어떻게 동작하나요? , CTRL-D?
```
...
• 싱글 정적변수(Single static variable)로 get_next_line 성공하는 것.
